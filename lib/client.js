...
export class RealtimeClient extends RealtimeEventHandler {
   ...
   * PATCHED BY MGW: Added transcription and model parameters
   * @param {{model?: string, transcription?: boolean}} [settings]
   * @returns {Promise<true>}
   */
  async connect({ model, transcription } = { model: 'gpt-4o-realtime-preview-2024-10-01', transcription: false }) {
    if (this.isConnected()) {
      throw new Error(`Already connected, use .disconnect() first`);
    }
    await this.realtime.connect({ model, transcription });
    // PATCHED: This was causing the session to be reset, but we want to keep the session.
    //this.updateSession();
    return true;
  }
  ...
  appendInputAudio(arrayBuffer) {
    if (arrayBuffer.byteLength > 0) {
      ...
      // PATCHED BY MGW: This somehow caused the app to crash.
